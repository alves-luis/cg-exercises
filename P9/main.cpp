...
void computeNormal(int i, int j) {
	...
	// points for v1
	float p11x = i - imageWidth * 0.5f;
	float p11y = h(i,j+1);
	float p11z = j - imageWidth * 0.5f + 1;

	float p12x = p11x;
	float p12y = h(i,j-1);
	float p12z = j - imageWidth * 0.5f - 1;
	...
	// vectors
	float v1[3] = { p11x - p12x, p11y - p12y, p11z - p12z};
	float v2[3] = { p21x - p22x, p21y - p22y, p21z - p22z};

	float crossR[3];
	cross(v1,v2,crossR);
	normalize(crossR);
	normal.push_back(crossR[0]);
	normal.push_back(crossR[1]);
	normal.push_back(crossR[2]);
